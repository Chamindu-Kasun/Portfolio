...
export async function POST(request: Request) {
  try {
    ...
    const thankYouEmailContent = generateThankYouEmailContent(
      `${firstname} ${lastname}`
    );
    ...
    console.error("Error processing the form submission:", error);
    return new NextResponse("Error processing the form submission", {
      status: 500,
    });
  }
}
